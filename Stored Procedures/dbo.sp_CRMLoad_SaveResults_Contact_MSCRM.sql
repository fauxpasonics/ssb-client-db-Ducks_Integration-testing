...
--DECLARE	@maxcopyloaddate datetime = (SELECT MAX(copyloaddate) FROM Prodcopy.Contact WITH (NOLOCK))

--UPDATE prodcopy.contact 
--SET statecode = 1, copyloaddate = @maxcopyloaddate
--FROM Prodcopy.contact pcc
--INNER JOIN dbo.Contact_CRMResults cr
--ON CAST(pcc.contactid AS varchar(50)) = cr.contactid
--WHERE cr.CrmErrorMessage LIKE 'CRM service call returned an error: contact With Id = % Does Not Exist'
GO
