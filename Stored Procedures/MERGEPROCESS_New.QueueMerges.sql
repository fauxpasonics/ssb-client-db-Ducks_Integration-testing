...
CREATE PROC [MERGEPROCESS_New].[QueueMerges] --ducks
--Exec  [MERGEPROCESS_New].[QueueMerges] 'ducks'
    @Client VARCHAR(100) 
AS
...
DECLARE @Contact VARCHAR(100) = (SELECT CASE WHEN @client = 'ducks' THEN 'CRM_Contact' ELSE CONCAT(@client,' CRM Contact' ) END ); --updateme
DECLARE @Account VARCHAR(100) = (SELECT CASE WHEN @client = 'ducks' THEN 'CRM_Account' ELSE CONCAT(@client,' CRM Account' ) END ); --updateme
--WITH only2Acct as
...
WHERE 
((AutoMerge = 1 and NumRecords = 2
) OR Approved = 1)
    AND 
	b.[ObjectType] = 'Contact'
    AND MergeComplete = 0 
GROUP BY PK_MergeID,b.[ObjectType]
HAVING MAX(CASE WHEN xRank =1 THEN CAST(c.ID AS VARCHAR(100)) END) IS NOT NULL AND  MAX(CASE WHEN xRank =2 THEN CAST(c.ID AS VARCHAR(100)) END) IS NOT NULL
